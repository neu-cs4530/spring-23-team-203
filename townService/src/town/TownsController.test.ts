...
describe('TownsController integration tests', () => {
  ...
  describe('Polls', () => {
    ...
    let sessionToken: string;
    let polls: Poll[];
    const pollId: string = "testpollid";
    const question: string = "What are you thinking about?";
    ...
    describe('Create Polls', () => {
      ...
      it('Returns an error message if the town ID is invalid', async () => {
        await expect(controller.createPoll(
          nanoid(),
          sessionToken,
          {question, options, settings}
        )).rejects.toThrow();
    ...
    describe('Vote in Polls', () => {
      ...
      it('Returns an error message if the town ID is invalid', async () => {
        ...
        await expect(controller.voteInPoll(
          nanoid(),
          pollId,
          sessionToken,
          {userVotes: [0]}
    ...
    describe('Get All Polls', () => {
      ...
      it('Successfully gets all polls when there are none', async () => {
        const polls = await controller.getAllPolls(
          testingTown.townID,
          sessionToken,
        );
      ...
      it('Successfully gets all polls when there is one', async () => {
        ...
        let pollsContents = Object.assign(polls[0]);
        delete pollsContents.creatorId;

        expect(polls).toHaveLength(1);
        expect(pollsContents).toMatchObject({pollId: poll.pollId,
      ...
      it('Successfully gets all polls when there are multiple', async () => {
        ...
        const poll2 = await controller.createPoll(
          testingTown.townID,
          sessionToken,
          {question: "Why do you think?", options, settings}
        );
        const poll3 = await controller.createPoll(
          testingTown.townID,
          sessionToken,
          {question, options, settings: {anonymize: true, multiSelect: true}}
        );
        const polls = await controller.getAllPolls(
          testingTown.townID,
          sessionToken,
      ...
      it('Throws error if town ID is invalid', async () => {
        ...
        await expect(controller.getAllPolls(
          nanoid(),
          sessionToken,
        )).rejects.toThrow();
      });
    ...
    describe('Delete Poll', () => {
      ...
      it('Successfully deletes poll when there is only one', async () => {
        ...
        let polls = await controller.getAllPolls(
          testingTown.townID,
          sessionToken,
        );
        expect(polls).toHaveLength(1);

        await controller.deletePoll(
          testingTown.townID,
          poll.pollId,
          sessionToken,
        );

        polls = await controller.getAllPolls(
          testingTown.townID,
          sessionToken,
        );
        expect(polls).toMatchObject([]);
      ...
      it('Throws error when trying to delete nonexistent poll', async () => {
        await expect(controller.deletePoll(
          testingTown.townID,
          nanoid(),
          sessionToken,
        )).rejects.toThrow();
